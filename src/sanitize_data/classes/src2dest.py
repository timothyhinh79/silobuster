...
class Src2Dest(object):
    ...
    def set_source_conn(self, source_conn_str = ''):
        if not source_conn_str: return
        source_db = sqlalchemy.create_engine(source_conn_str, echo=True)
        source_conn = source_db.connect()
        self.source_db = source_db
        self.source_conn = source_conn

    def set_dest_conn(self, dest_conn_str = ''):
        if not dest_conn_str: return
        dest_db = sqlalchemy.create_engine(dest_conn_str, echo=True)
    ...
    def create_dest_table(self):
        ...
        if self.dest_conn.engine.url != self.source_conn.engine.url:
            ...
            self.dest_conn.execute(
                sqlalchemy.sql.text(f"DROP TABLE IF EXISTS {self.source_table}; DROP TABLE IF EXISTS {self.source_table}_src;")
            )
            ...
            self.dest_conn.execute(
                sqlalchemy.sql.text(f"ALTER TABLE {self.source_table} RENAME TO {self.source_table}_src;")
            )
